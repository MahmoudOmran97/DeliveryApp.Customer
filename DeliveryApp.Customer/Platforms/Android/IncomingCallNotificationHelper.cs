...
internal static class IncomingCallNotificationHelper
{
    ...
    // notifId ثابت لكل orderId عشان نقدر نلغي نفس النوتيفيكيشن من الـ BroadcastReceiver بسهولة
    ...
    public static void Show(Context context, int orderId, string callerName)
    {
        try
        {
            ...
            var builder = new NotificationCompat.Builder(context, CallChannelId)
                ...
                .SetAutoCancel(true)
                .SetOngoing(true) // ميتمسحش لوحدها من السحب عادي، لازم رد/رفض/تايم آوت
            ...
            {
                // ده اللي بيخلي النوتيفيكيشن تظهر full-screen حتى لو الشاشة مقفولة (زي مكالمة عادية)
